...
if __name__ == "__main__":
    cascade = torch.randn(3,50,64)
    soure = cascade[:, 0, :]

    ones = torch.ones(3,30)
    zeros = torch.zeros(3,20)
    input = torch.cat([ones, zeros], -1)
    mask = (input == 0)

    att = Long_term_atention(64)
    out = att(cascade,soure, cascade, mask)
