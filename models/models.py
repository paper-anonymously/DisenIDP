...
class LSTMGNN(nn.Module):
    def __init__(self, hypergraphs, args, dropout=0.2):
        ...
        self.conv = ConvBlock(n_inputs=self.emb_size, n_outputs=self.emb_size, kernel_size=self.win_size,
                              padding=self.win_size - 1)
        self.fusion = fusion(self.emb_size)
    ...
    def seq2seq_ssl(self, L_fea1, L_fea2, S_fea1, S_fea2):
        ...
        neg1 = score(L_fea1, row_shuffle(S_fea2))
        neg2 = score(L_fea2, row_column_shuffle(S_fea1))
        Long_loss = torch.sum(-torch.log(torch.sigmoid(pos - neg1)) - torch.log(torch.sigmoid(neg1 - neg2)))

        pos = score(S_fea1, S_fea2)
        neg1 = score(S_fea1, row_shuffle(L_fea1))
        neg2 = score(S_fea2, row_column_shuffle(L_fea2))
        Short_loss = torch.sum(-torch.log(torch.sigmoid(pos - neg1)) - torch.log(torch.sigmoid(neg1 - neg2)))
    ...
    def forward(self, input, label, input_time, label_time):
        ...
        # LS_cas_emb = self.fusion(cas_seq_emb, L_cas_emb, S_cas_emb)
        output = self.past_multi_att(L_cas_emb, L_cas_emb, L_cas_emb, mask)
        # pre_y = self.linear2(output)
        ...
        '''SSL loss'''
        # graph_ssl_loss = self.hierarchical_ssl(self.self_supervised_gating(HG_Uemb, 0), self.H_Time)
        ...
        seq_ssl_loss = 0
        # seq_ssl_loss += self.seq2seq_ssl(S_cas_emb, short_emb, mask_label)
