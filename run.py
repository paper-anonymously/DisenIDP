...
def model_training(model, train_loader, epoch):
    ...
    with tqdm(total=len(train_loader)) as t:
        for step, (cascade_item, label, cascade_time, label_time, cascade_len) in enumerate(train_loader):
            ...
            pred, ssl_loss, ss_loss2 = model(cascade_item, tar, cascade_time, label_time)
...
def main(data_path, seed=2023):
    ...
    HG_Item, HG_User = ConHypergraph(opt.data_name, opt.n_node)  ###Item Hypergraph, Source User
    # HG_Time = trans_to_cuda(HG_Time)
